...
try:
    ...
    win32api = None  # 在未安装 pywin32 的环境下降级处理

from matplotlib.pyplot import savefig


class SlamSleepAwakeTester:
    ...
    def run_adb_command(self, command, capture_output=True, timeout=10):
        """执行ADB命令并返回结果"""
    ...
    def device_reboot(self):
        """重启设备"""
        self.run_adb_command("reboot", capture_output=False)
    ...
    def run_test_cycle(self):
        ...
        try:
            ...
            if init_success:
                ...
                self.send_dingtalk_alert(f'第 {self.test_count} 次休眠唤醒后初始化失败')
                # return False



            # 检查重定位
            relocal_success = self.check_log_for_keyword("Merge global map done")
            ...
            # 检查是否出现QNN_ERROR
            # relocal_success = self.check_log_for_keyword("Dma execution failed")
            # self.sheet.cell(row, 3, value="PASS" if relocal_success else "FAIL")
            # # print(f"QNN: {'复现！！！' if relocal_success else '未复现'}")
            # if relocal_success:
            #     self.save_logs("QNN_ERROR")
            #     self.send_dingtalk_alert(f'第 {self.test_count} 次休眠唤醒后QNN_ERROR')
            #     return False
            # else:
            #     print(f'QnnError:未复现')
            ...
            if has_crash:
                ...
                self.send_dingtalk_alert(f'第 {self.test_count} 次休眠唤醒后crash')
    ...
    def continuous_testing(self):
        """连续执行测试"""
        print(f"开始SLAM休眠唤醒测试，设备: {self.config['device_id']}")
...
if __name__ == "__main__":
    # 配置参数统一从 slam_config 引入，便于维护
    from slam_config import SLEEP_CONFIG as config
    ...
    tester = SlamSleepAwakeTester(config)
