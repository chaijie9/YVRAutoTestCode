...
Excel_File = r"./sleep_RC_D3_3.0.1.33_no_mes.xlsx"
if not os.path.exists(Excel_File):
    wb = openpyxl.Workbook()
    wb.create_sheet(title=str("Slam_Log_MS"), index=0)
    wb.save(Excel_File)
workbook1 = openpyxl.load_workbook(Excel_File)
sh = workbook1["Slam_Log_MS"]
...
while True:
    ...
    subprocess.call("adb -s D3HDXD2D4363000102 reboot ")
    ...
    # print("开MESH")
    start_modeC = "adb -s D3HDXD2D4363000102 shell am broadcast -a com.yvr.demo.action.dev.mode --include-stopped-packages"
    print(start_modeC)
    subprocess.call(start_modeC, shell=True, timeout=15)
    time.sleep(3)
    root = "adb -s D3HDXD2D4363000102 shell setprop service.dev.mode 1"
    print(root)
    ...
    print("start")
    time.sleep(8)
    init_command = 'adb -s D3HDXD2D4363000102  logcat -d | findstr "delivered."'
    ...
    recenter_command = 'adb -s D3HDXD2D4363000102 logcat -d | findstr "Merge"'
    p_obj = subprocess.Popen(
        args=recenter_command,
        stdin=None, stdout=subprocess.PIPE,
        stderr=subprocess.PIPE, shell=True)
    ...
    if msg.find("mapping Merge global map") >= 0:
        ...
        subprocess.call(f"adb -s D3HDXD2D4363000102 logcat -d > ./LOG/{now_time}_{test_count}.txt", shell=True)

    print(f"第", test_count, "次")
    result_tomb = adb_command("adb -s D3HDXD2D4363000102  shell ls data/tombstones/")
    result_anr = adb_command("adb -s D3HDXD2D4363000102  shell ls data/anr/")
    print(result_tomb, result_anr)


    if result_tomb.find("_") > 0 or result_anr.find("_") > 0:
        now_time = time.strftime("%H_%M_%S")
        subprocess.call(f"adb -s D3HDXD2D4363000102 logcat -d > ./LOG/crash_{now_time}_{test_count}次.txt", shell=True)
        subprocess.call(f"adb -s D3HDXD2D4363000102  pull /data/tombstones  ./tomb/{now_time}")
        time.sleep(2)
        subprocess.call("adb -s D3HDXD2D4363000102  shell rm -rf /data/tombstones/*", shell=True)
        subprocess.call("adb -s D3HDXD2D4363000102  shell rm -rf /data/anr/*", shell=True)
        time.sleep(2)
