...
# def get_cpu_usage():
#     # 运行 adb shell top 命令，并将输出保存在变量中
#     output = subprocess.check_output(['adb shell top -d 1 | findstr tracking'])
#
#     # 解析输出，提取 CPU 占用率信息
#     lines = output.decode().split('\n')
#     cpu_line = [line for line in lines if line.startswith('CPU')][0]
#     cpu_info = cpu_line.split()[1:]
#
#     # 提取各个 CPU 核心的占用率
#     cpu_usage = [float(usage.strip('%')) for usage in cpu_info]
#
#     return cpu_usage

# 记录 CPU 占用率到文件
def record_cpu_usage(filename, duration, interval):
...
# 示例用法：记录 CPU 占用率到文件 cpu_usage.txt，持续时间 60 秒，每秒记录一次
record_cpu_usage('./thread.txt.txt', 10, 1)